...
"""
    ...
    val conditions = condition.prods.flatMap { prod =>
      val expressions = prod.exps.collect {
        case Comparison(op, index, variable) =>
          s"${C_convert_index(index)} ${
              if op == "=" then "==" else op
            } ${variable.name}"
      }
      if expressions.nonEmpty then Some(expressions.mkString(" && ")) else None
    }

    val flag = getVar("flag")
    val flagCode =
      if conditions.nonEmpty then
        s"int $flag = ${conditions.mkString(" || ")};\n"
      else s"int $flag = 0;\n"
    (flag, flagCode)
  ...
  def C_alloc_and_gen_random_number(
    ...
    dims match
      ...
      case _   =>
        ...
        val c0 =
          s"double ${"*" * dimensions.length} $var_name = malloc(sizeof(double ${"*" *
              (dimensions.length - 1)}) * ${dimensions.head});\n"
        val c1 = dimensions.zip(iter_seq).zipWithIndex.map {
          case ((dim, i), n) =>
            val c_sub1 = s"for (size_t $i = 0; $i < $dim; ++$i) {\n"
            val c_sub2 =
              if n != dimensions.length - 1 then
                s"$var_name[${iter_seq.slice(0, n + 1).mkString("][")}] = malloc(sizeof(double " +
                  "*" * (dimensions.length - 2 - n) +
                  s") * ${dimensions(n + 1)});\n"
              else ""
            c_sub1 + c_sub2
        }.mkString("\n")
  ...
  def CPP_alloc_and_gen_random_number(
    ...
    dims match
      ...
      case _   =>
        ...
        val c0 =
          s"double " + "*" * dimensions.length + s"$var_name = new double" +
            "*" * (dimensions.length - 1) + s"[${dimensions.head}];\n"
        val c1 = dimensions.zip(iter_seq).zipWithIndex.map {
          case ((dim, i), n) =>
            val c_sub1 = s"for (size_t $i = 0; $i < $dim; ++$i) {\n"
            val c_sub2 =
              if n != dimensions.length - 1 then
                s"$var_name[${iter_seq.slice(0, n + 1).mkString("][")}] = new double" +
                  "*" * (dimensions.length - 2 - n) +
                  s"[${dimensions(n + 1)}];\n"
              else ""
            c_sub1 + c_sub2
  ...
  def C_printerr(access: Access): String = access.vars.isEmpty match
    ...
    case false =>
      s"""fprintf(stderr, "%f\\n", ${access.name}[${access.vars.map(e => s"0")
          .mkString("][")}]);"""

  def CPP_free(var_name: String, dims: Seq[Dim]) =
    val dimensions = dims.map(C_convert_index(_))
    val c0 = dimensions.init.zipWithIndex.map { case (dim, i) =>
      s"for (size_t i$i = 0; i$i < $dim; ++i$i) {"
    }.mkString("\n")
    val iter_seq: Seq[String] = dimensions.tail.zipWithIndex.map {
      case (_, id) => s"i$id"
    }
    val c1 = iter_seq.zipWithIndex.map { case (iter, i) =>
