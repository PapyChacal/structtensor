...
object Codegen:
  ...
  def CPPFormat(a: Any): String =
    a match
      case s: String =>
        if s.endsWith("^-1") then s"1. / ${s.dropRight(3)}" else s
      ...
      case a: Access =>
        if a.vars.isEmpty then CPPFormat(a.name)
        else s"${CPPFormat(a.name)}[${a.vars.map(_.name).mkString("][")}]"
  ...
  def getConditionsOnVariable(
    ...
    (
      ...
      conditions.zipWithIndex.filterNot { case (_, ind) =>
  ...
  def reorder(
        ...
        if variables.contains(v) then
          i match
            case va: Variable if (variables.contains(va)) =>
              if variables.indexOf(va) < variables.indexOf(v) then
                (v -> Seq(va), None)
              else (va -> Seq(v), None)
            case _ =>
              (
                v -> getVariablesInIndex(i).filter(variables.contains),
                getVariablesInIndex(i).filter(variables.contains)
                  .map(_ -> Seq(v)).toMap,
              )
        else if symbols.contains(v) then
          i match
            case va: Variable if (variables.contains(va)) =>
              (va -> Seq(v), None)
            case _ => (None, None)
    ...
    val afterMap1 = afterMapInit.filterNot(_ == None).groupBy {
      case (k: Variable, _) => k
    }.mapValues(_.flatMap { case (_: Variable, vars: Seq[Variable]) => vars })
      .map { case (k, v) => k -> v.distinct }
      .toMap // .foreach { case (k, v) => println(s"$k -> ${v}") }
    val removeIfExist = removeIfExistInit.filterNot(_ == None)
      .filterNot(_ == Map()).toSeq.flatten.groupBy { case (k: Variable, _) =>
        k
      }.mapValues(_.flatMap { case (_: Variable, vars: Seq[Variable]) => vars })
      .map { case (k, v) => k -> v.distinct }.toMap
    ...
    def reorderRec(
      ...
      val newIndexMap = afterMap.map { case (k, v) =>
        if !v.filter(variables.contains).isEmpty then
      ...
      if sortedNewIndexMap == indexMap then
        indexMap.toSeq.sortBy(_._2).map(_._1)
  ...
  def codeGenSingleProd(
    ...
    val (loopNests, restOfConditions, numberOfBrackets1) = variables.reverse
      .foldLeft(
        Seq[String](),
        conditions,
        0,
        ...
        val eqCode = equals.map(e =>
          e match
            case va: Variable =>
              if (!begin.isEmpty || !end.isEmpty) && !symbols.contains(va) then
        ...
        codeLang.toLowerCase() match
          case "cpp" | "c++" =>
            (
              ...
              case (1, 1, false) =>
                if end.mkString == s"(${begin.mkString}) + 1" then
                  (
                    acc._1 ++ Seq(
                      eqCode,
                  ...
                  (
                    acc._1 ++ Seq(
                      s"if (${CPPFormat(v)} >= ${begin
                          .mkString} && ${CPPFormat(v)} < ${end.mkString}) {",
                      eqCode,
                    ),
                    rest,
                    acc._3 + 1,
            ...
            (
              ...
              case (1, _, true) =>
                ...
                (
                  acc._1 ++ Seq(
                    s"if (${CPPFormat(v)} >= ${begin
                        .mkString} && ${CPPFormat(v)} < min($arr, ${end.length})) {",
                    eqCode,
                    c1,
                  ),
                  rest,
              ...
              case (_, 1, true) =>
                ...
                (
                  acc._1 ++ Seq(
                    s"if (${CPPFormat(v)} >= max($arr, ${begin
                        .length}) && ${CPPFormat(v)} < ${end.mkString}) {",
                    eqCode,
                    c1,
                  ),
                  rest,
  ...
  def apply(
    ...
    rule.body.prods.zipWithIndex.map { case (prod, ind) =>
      val conditions =
