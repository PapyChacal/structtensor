  ...
  def main(args: Array[String]): Unit =
    ...
    val parser1 =
      ...
      OParser.sequence(
        ...
        opt[String]('l', "lang").action((x, c) => c.copy(codeLang = x))
          .text("select the target language for the generated code")
          .valueName("<lang>"),
        opt[String]('i', "inf").action((x, c) => c.copy(inFilePath = x))
          .text("path to the stur input file").valueName("<path>"),
        opt[String]('o', "outf").action((x, c) => c.copy(outFilePath = x))
          .text("where to stored the generated code").valueName("<path>"),
        opt[Unit]("init-tensors").action((_, c) => c.copy(initTensors = true))
    ...
    OParser.parse(parser1, args, Config()) match
      case Some(config) =>
        if config.inFilePath != "" then
          ...
          import Codegen.*
          val source =
            try scala.io.Source.fromFile(config.inFilePath)
            catch
          ...
          val symbols = symbols_lines.map(e => e.slice(8, e.length))
            .flatMap(_.split(",").map(_.trim).toSeq).map(Variable(_))
          // parsed outputs names
          val outputs_names = outputs_lines.map(e => e.slice(8, e.length))
            .flatMap(_.split(",").map(_.trim).toSeq)
          // parsed iterators names and vars
          val iters_map = iters_lines.map(e => e.slice(6, e.length))
            .flatMap(_.split(";").map(_.trim).toSeq).map(iter_str =>
              fastparse.parse(iter_str, Parser.iterators(using _)).get.value
            ).toMap
          // Remaining input lines AKA the program + compression hatches
          val lineSeq = lineSeqInit.zipWithIndex.filterNot(x =>
            ...
            tensorComputations_computation.foldLeft(
              ...
              (
                acc._1 + (usRule.head -> usRule) +
                  (usRule.head.inverseHead() -> usRule),
                acc._2 +
                  (Access(
                    rmRule.head.name,
                    usRule.head.vars,
                ...
                acc._3 + (ccRule.head -> ccRule) +
                  (ccRule.head.inverseHead() -> ccRule.inverse()),
                acc._4 :+ ccRule,
                acc._5 :+ rcRule,
              )
            }
          ...
          if config.codeLang == "MLIR" then
            ...
            val outputNames = outputs_names match
              ...
              case names => names
            val outputs = outputNames
              .map(n => (name = n, rank = dimMap(n).length))

            val mlirGen = MLIRGen(symbols, iters_map, dimMap, outputs)
            val module = ModuleOp(
              Region(
                mlirGen.genCompute(ccRuleSeq) +:
                  (mlirGen.genReconstruct(rcRuleSeq) ++
                    mlirGen.genMain(ccRuleSeq))
              )
            )
          ...
          val ccComputation = outputs_names.isEmpty match
            ...
            case false =>
              ccRuleSeq.filter(r => outputs_names.contains(r.head.name))
                .map(r =>
                  Codegen(
                    r,
                    symbols,
          ...
          config.onlyComputation match
            ...
            case false =>
              val rcComputation = outputs_names.isEmpty match
                case true =>
                  rcRuleSeq.map(r =>
                    Codegen(r, symbols, config.codeLang, RedundancyMap)
                  ).mkString("\n")
                case false =>
                  rcRuleSeq.filter(r => outputs_names.contains(r.head.name))
                    .map(r =>
                      Codegen(r, symbols, config.codeLang, RedundancyMap)
                    ).mkString("\n")
              write2File(
        ...
        else println("Please specify the stur code or the file path")
      case _ =>
