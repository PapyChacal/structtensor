  ...
  def generateInit(
    ...
    initTensors match
      case true =>
        ...
        val argv_names =
          if symbols.nonEmpty then symbols.map(_.name)
          else
        ...
        val c3 = all_tensors.map(_.deinversifiedHead()).distinctBy(_.name)
          .filter(_.kind == Tensor).filterNot(only_lhs_heads.contains)
          .filterNot(t => symbols.contains(t.name.deinversifiedName.toVar))
          .filterNot(t => decimal_pattern.matches(t.name)).map(t =>
        ...
        val c4 = outputs_names.isEmpty match
          ...
          case false =>
            only_lhs_heads.filter(e => outputs_names.contains(e.name)).map(t =>
              alloc_and_gen_zero(
                codeLang,
                t,
                all_dimensions.getOrElse(t, Seq[Dim]()),
    ...
    initTensors match
      case true =>
        ...
        val c1 = outputs_names.isEmpty match
          case true =>
            all_tensors.map(_.deinversifiedHead()).distinctBy(_.name)
              .map(t => printerr(codeLang, t)).mkString("\n")
          case false =>
            all_tensors.filter(e => outputs_names.contains(e.name))
              .map(t => printerr(codeLang, t.deinversifiedHead()))
              .mkString("\n")
        val c2 = all_tensors.distinctBy(_.name)
          .filterNot(only_lhs_heads_not_in_output.contains)
          .map(_.deinversifiedHead()).distinctBy(_.name)
          .filter(all_dimensions.contains)
          .map(t => free(codeLang, t.name, all_dimensions(t))).mkString("\n")
